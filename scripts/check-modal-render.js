...
(async () => {
  ...
  const screenshotPath = path.join(outDir, 'modal-check.png');
  ...
  try {
    ...
    await page.waitForSelector('.portfolio-card', { timeout: 8000 });
    // click first card
    await page.click('.portfolio-card');
    await page.waitForSelector('#project-modal[aria-hidden="false"]', { timeout: 5000 });
